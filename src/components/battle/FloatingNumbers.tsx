  ...
  return (
    <div className="floating-numbers">
      {numbers.map((n) => (
        <span
          key={n.id}
          className={`floating-number floating-number-${n.kind}${n.emphasis ? ' floating-number-emphasis' : ''}${n.kind === 'damage' && n.tier ? ` floating-number-l${n.tier}` : ''}${n.bonus ? ' floating-number-bonus' : ''}${n.max ? ' floating-number-max' : ''}`}
          style={{
